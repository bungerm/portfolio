...
void ParticleEmitter::SpawnParticle()
{
	...
	if (last_active_particle < max_particles - 1)
	{
		...
		newParticle->scale = Vect4D(2.0, 2.0, 2.0, 2.0);
...
void ParticleEmitter::draw()
{
	...
	for (it = drawBuffer.begin(); it != drawBuffer.end(); ++it)
	{
		...
		transParticle.setTransMatrix(&(it->position * 0.35));
...
void ParticleEmitter::Execute(Vect4D& pos, Vect4D& vel, Vect4D& sc)
{
	...
	// x - variance
	double var = static_cast<double>(rand() % 1000) * 0.005f; // Var
	...
	*t_pos += *t_var * var;
	...
	// correct the sign
	var = 1.5f * static_cast<double>(rand() % 1000) * 0.001f;
	sign = static_cast<double>(rand() % 2);  
